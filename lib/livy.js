 ...
 */
export async function runService({ serviceId, params = {}, withAttestation = true, postToDataAvailability = false }) {
  try {
    // Make request to our Next.js API proxy instead of direct SDK call
    const response = await fetch('/api/livy/run', {
      method: 'POST',
      headers: {
    ...
    return {
      ...
      dataAvailabilityResult: data.dataAvailabilityResult,
    };
...
/**
 * Gets a blob from Celestia through the Next.js API proxy
 * @param {Object} config - Configuration object with celestiaHeight, celestiaCommitment
 * @returns {Promise<any>} Result from the API call
 * @throws {Error} When API call fails
 */
export async function getBlob({ celestiaHeight, celestiaCommitment }) {
  try {
    const response = await fetch('/api/livy/getBlob', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
